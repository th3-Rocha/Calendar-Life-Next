...
import { Info, ListTodo, User, X } from "lucide-react";
import { useParams } from "next/navigation";
import { toast } from "sonner";
import { useLifeData, type SyncStatus } from "../../hooks/useLifeData";
...
export default function Home() {
  ...
  const lastSyncStatusRef = useRef<SyncStatus | null>(null);

  useEffect(() => {
    if (syncStatus === lastSyncStatusRef.current) return;
    ...
    if (syncStatus === "idle") {
      if (toastIdRef.current !== null) {
        toast.success("Saved to cloud!", { id: toastIdRef.current });
    ...
    if (syncStatus === "error") {
      if (toastIdRef.current !== null) {
        toast.error("Sync failed (Offline?)", { id: toastIdRef.current });
        toastIdRef.current = null;
      } else {
        toast.error("Sync failed (Offline?)");
      }
    }
  }, [syncStatus]);
